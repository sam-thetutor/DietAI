...
interface MealPlanRequest {
    ...
    allLoggedFoods: string[]; // List of all unique foods ever logged
}

interface MealPlanResponse {
    breakfast: string[]; // Array of 5 breakfast suggestions
    lunch: string[];     // Array of 5 lunch suggestions
    supper: string[];    // Array of 5 supper suggestions
}
// --- End Types ---
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { healthGoal, calorieTarget, allLoggedFoods } = body;

    // Basic input validation
    if (!healthGoal || !calorieTarget || typeof calorieTarget !== 'number' || calorieTarget <= 0 || !Array.isArray(allLoggedFoods)) {
      return NextResponse.json({ error: 'Missing or invalid input data (goal, target, food list).' }, { status: 400 });
    }
    ...
    const generationConfig = {
      temperature: 0.8, // Higher creativity for meal ideas
      topK: 40,
      topP: 0.95,
      maxOutputTokens: 2048, // Need more tokens for a full plan
    ...
    const prompt = `
        ...
        - Daily Calorie Target: Approximately ${calorieTarget} kcal
        ...
        - Suggestions should align with the user's health goal (${healthGoal}).
        ...
        - Keep suggestions relatively simple and practical. Include brief descriptions or key ingredients. Avoid just listing single items unless it's a typical simple meal (e.g., "Oatmeal with berries").
        - Do NOT include calorie counts for each meal suggestion in the output. Focus on the meal idea itself.
    ...
    try {
           ...
           )
        {
            ...
            return NextResponse.json(jsonResponse); // Return the validated plan
        } else {
